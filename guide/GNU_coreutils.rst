...
man page の自動生成の環境が整います。ただし、coreutils をそのまま make すると、
arch,coreutils,hostname,kill,uptime が自動生成されません。そのため、個別に
バイナリを作成する、make stamp-build を先に実行してください。
なお、生成されないプログラムは coreutils のバージョンによって異なります。
バージョン 9.11 は前述の5つです。
...
* トップディレクトリで ``make install`` を実行すると、
  ``help2man.ja/*.1`` が ``draft/man1`` にコピーされます。

翻訳作業
--------

``source/po/ja.po`` と ``help2man.ja/*.x`` を編集して翻訳を行います。
ファイルの詳細は :ref:`下記 <coreutils-files>` を参照してください。
